...
import { Path, Point } from './mask_util';
...
const splitPointKey = (point: string) => {
  return point.split(' ').map((a: string) => parseInt(a)) as Point;
};
 ...
 * @returns {Path[]} path array with points[x,y] for display
 */
export const convertSegmentsToPathList = (polySegments: any) => {
  ...
  while (polySegments.size) {
    ...
    const path: Path = [splitPointKey(firstPoint)];
...
/**
 * Check the nested paths, alternate the direction if the path is the holes of other path,
 * the logic is relocated from convertSegmentsToSVG function to here, to avoid the canvas
 * reference, because we want to move the convertSegmentsToSVG to web worker to improve
 * effciency
 *
 * @param {Path[]} pathList the path segment list
 * @returns {Path[]}
 */
export const reverseSVGSegments = (pathList: Path[]) => {
  // 2. Compute desired direction for each path, flip if necessary
  const renderedPaths = [];

  // We use a canvas element to draw the paths and check isPointInPath to determine wanted direction
  // So we can not run these code in web worker
  const canvas = document.createElement('canvas');
  const ctx = canvas.getContext('2d');
